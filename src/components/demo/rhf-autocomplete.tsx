...
import { RHFAutocomplete } from "@/components/ui/hook-form";
import FormProvider from "@/components/ui/hook-form/form-provider";
import { useForm } from "react-hook-form";
import { zodResolver } from "@hookform/resolvers/zod";
import { z } from "zod";
import { Button } from "@/components/ui/buttons/button";
import { ClapperboardIcon, Loader } from "lucide-react";
...
export default function RHFInputDemo() {
	...
	const handleCreate = (newOption: Option): Option => {
		return {
		...
		<FormProvider {...methods} onSubmit={handleSubmit} className="w-full flex flex-col items-center justify-center">
			<div className="max-w-72 w-full">
				<RHFAutocomplete<TFormSchema>
					...
					asynFilterFunction={fetchData}
					loadingLabel={
						<div className="inline-flex items-center">
							<Loader className="mr-1.5 animate-spin duration-2000 size-5" />
							<span className="text-sm">Fetching movies...</span>
						</div>
					}
				/>
			</div>
